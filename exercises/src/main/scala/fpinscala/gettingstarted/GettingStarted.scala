...
object MyModule {
  ...
  def fibIter(n: Int): Int = {
    def fib(count: Int, current: Int, next: Int): Int = {
      if (count == 0) current
      else {
        fib(count - 1, next, current + next)
      }
    }
    fib(n, 0, 1)
  }
...
object TestFib {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("Actual tail:   %d, %d, %d, %d, %d, %d, %d".format(fibIter(0), fibIter(1), fibIter(2), fibIter(3), fibIter(4), fibIter(5), fibIter(6)))
  }
}
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    as.zip(as.tail).map(elm => !gt(elm._1, elm._2)).reduce((x, y) => x && y)
  }
